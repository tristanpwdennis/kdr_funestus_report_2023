...
df_samples = fread('~/Projects/kdr_funestus/df_samples.csv')
...
#  geom_sf(data=st_geometry(ocean_shape), colour = '#4a80f5', fill='#9bbff4')+
  ...
  geom_point(data=tzdf, aes(y=as.numeric(admin1_rep_long), x=as.numeric(admin1_rep_lat), colour=admin1_rep),  alpha = 0.8, size=4)+#colour=query))+#size=sequenced_count))+
  ...
  xlim(30, 40.5)+
  ylim(-11.6,-0.8)+
  labs(x="Longitude", y="Latitude", colour='Admin1_Cohort_Year_Month')+
  theme_minimal()+
